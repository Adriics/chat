...
export const ValueObjectTransformer = <T extends Primitives>(
  ...
  to: (value?: Nullable<ValueObject<T>>): Nullable<T> =>
    isNullOrUndefined(value) ? null : value.valueOf(),
  from: (value: Nullable<T>): Nullable<ValueObject<T>> =>
    isNullOrUndefined(value) ? null : new ValueObjectClass(value.valueOf()),
})
