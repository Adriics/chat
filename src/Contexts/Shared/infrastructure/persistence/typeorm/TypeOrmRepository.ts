...
import { DataSource, Repository, SelectQueryBuilder, EntitySchema } from 'typeorm'
...
export abstract class TypeOrmRepository<T extends AggregateRoot> {
    ...
    if (criteria.order) {
      ...
      for (const order of orders) {
        if (order.orderType.valueOf() !== OrderTypes.NONE) {
          queryBuilder.addOrderBy(String(`"${order.orderBy}"`), String(order.orderType).toUpperCase() as 'ASC' | 'DESC')
        }
      }
    }
  ...
  private convertInValues(values: FilterValue[]): (string | number | boolean)[] {
    ...
    if (!isNaN(Number(values[0].valueOf())))
      return values.filter(x => !isNaN(Number(x.valueOf()))).map(x => Number(x.valueOf()))
    if (
  ...
  protected static mapFilterOperatorToDatabaseOperator(operator: FilterOperator, value: Nullable<FilterValue>): string {
    const customMap = {
